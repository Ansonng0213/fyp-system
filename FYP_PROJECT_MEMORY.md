...
**[VALIDATION — MANUAL, KEY FRAMING] PlugShare per-zone check of the 4 Klang desert hexes (done 2026-08-01, recorded in `operator_crosscheck_template.csv` plugshare_* columns):**
- Manually verified live PlugShare (public + operational; restricted-access & coming-soon excluded) for the 4 worst Klang desert hexes. Result vs our snapshot count: **Z1 (3.01168,101.42758) our 0 → live ~5-6 | Z2 (2.98887,101.42099) our 1 → ~4-5 | Z3 (2.99837,101.39932) our 0 → ~0 (genuinely empty, nearest ~4km) | Z4 (3.01904,101.43276) our 0 → ~5-6.** Confidence HIGH.
- **KEY FINDING (map-evidenced):** the public charging that DOES exist in Klang sits at **commercial nodes — Aeon Mall Bukit Tinggi, GM Klang Wholesale City, Klang town centre, highway hotels (Premiere Hotel), Bukit Tinggi Medical Centre** — while the **surrounding residential tamans stay empty**. 3 of 4 hexes are not literally empty within 2km; the chargers cluster at the commercial fringe 0.5-1.8km from the residential cores. Z3 (Port Klang core) is the one genuinely empty (~4km to nearest).
- **FRAMING SHIFT (use this everywhere, incl. viva & final report):** DROP the literal "desert hexes have zero public stations within 2km" claim — live data weakens it. USE the stronger, defensible version: **"charging follows commercial-ROI siting, not residential need"** — exactly the project thesis, now directly evidenced on the map. This is a REFINEMENT, not a retraction: the district/aggregate desert evidence (2km pop-coverage Klang 47.2% vs KL 98%, per-100k 7.4× disparity, capacity per-port, 500m coverage 5.2% vs 33.4%) is untouched and still holds. The gap vs our snapshot is mostly recent commercial-venue build-out (snapshot-vs-live; "the pipeline is the product, the snapshot is the demo").
- Validation (Trust) page operator-cross-check section updated to present this as our-snapshot-vs-live-PlugShare with the commercial-clustering finding — NOT a raw zero-station claim.
