...
# ------------------------------------------------------------
# Cross-check template: the ONLY artifact in this pipeline that holds
# hand-entered data. Columns this stage computes are refreshed; everything
# else is carried across from the file on disk.
# ------------------------------------------------------------
COMPUTED_COLS = ["h3_index", "district", "lat", "lon", "zone_type",
                 "our_public_stations_2km", "maps_link"]
# Seeded blank by this stage for a human to fill in. Must NOT be re-blanked
# once filled, which is why they are carried, not regenerated.
MANUAL_SEEDED_COLS = ["jomcharge_count", "gentari_count", "chargev_count",
                      "other_count", "notes"]
TEMPLATE_ORDER = (COMPUTED_COLS[:-1] + MANUAL_SEEDED_COLS + ["maps_link"])


def _filled(s):
    """Cells a human actually put something in."""
    t = s.astype(str).str.strip().str.lower()
    return s.notna() & (t != "") & (t != "nan")


def merge_preserving_manual(fresh, path):
    """Refresh the computed columns, preserve every manual value.

    Rows are matched on h3_index. Any column this stage does not compute is
    carried across -- both the blank-seeded ones above and any column added
    to the file by hand later (e.g. the plugshare_* fieldwork), which this
    stage has never heard of and must not drop.
    """
    if not os.path.exists(path):
        print("  No existing template -- writing a fresh one.")
        return fresh[TEMPLATE_ORDER]

    # dtype=str so hand-typed values round-trip VERBATIM. Without it pandas
    # infers a count column as float and writes "7" back as "7.0".
    prev = pd.read_csv(path, dtype=str, keep_default_na=False)
    carry = [c for c in prev.columns if c not in COMPUTED_COLS]
    external = [c for c in carry if c not in MANUAL_SEEDED_COLS]

    out = (fresh[COMPUTED_COLS]
           .merge(prev[["h3_index"] + carry], on="h3_index", how="left"))
    for c in MANUAL_SEEDED_COLS:                      # guarantee they exist
        if c not in out.columns:
            out[c] = ""
    out[carry] = out[carry].fillna("")

    kept = int(sum(_filled(out[c]).sum() for c in carry))
    prev_filled = int(sum(_filled(prev[c]).sum() for c in carry))
    print(f"  Existing template found: {len(prev)} rows, {len(prev.columns)} columns.")
    print(f"  Carried across {len(carry)} non-computed column(s)"
          + (f", including {external} not generated by this stage" if external else ""))
    print(f"  MANUAL VALUES PRESERVED: {kept} of {prev_filled} found on disk")

    lost = prev[~prev["h3_index"].isin(fresh["h3_index"])]
    lost = lost[lost[carry].apply(lambda r: any(_filled(pd.Series([v]))[0] for v in r), axis=1)] \
        if len(lost) and carry else lost.iloc[0:0]
    if len(lost):
        print(f"  *** WARNING: {len(lost)} row(s) with manual data no longer match a")
        print("      selected zone (the zone selection changed) -- their values are")
        print("      NOT in the new file. Recover them from git before proceeding:")
        for h in lost["h3_index"]:
            print(f"        {h}")
    elif kept != prev_filled:
        print(f"  *** WARNING: {prev_filled - kept} manual value(s) did not carry across.")

    order = TEMPLATE_ORDER + [c for c in out.columns if c not in TEMPLATE_ORDER]
    return out[order]
...
# ---- MERGE, never clobber -------------------------------------------------
# This template carries HAND-ENTERED fieldwork (the ~1 hour of PlugShare
# verification). A plain to_csv() here silently destroyed it on every full
# pipeline run: the stage regenerates 12 columns, the file on disk had 15.
# Anything this stage does not compute is carried across, matched on h3_index.
zones = merge_preserving_manual(zones, os.path.join(OUT_DIR,
                                                    "operator_crosscheck_template.csv"))
